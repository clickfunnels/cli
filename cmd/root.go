...
// defaultClientID is the first-party OAuth client id baked into release builds
// (injected at build time via -ldflags "-X .../cmd.defaultClientID=<uid>"), so
// users are never asked for one. Empty in plain `go build`; override per login
// with --client-id or CF_CLI_CLIENT_ID.
var defaultClientID = ""
