...
- **The CLI is a public OAuth client** (`confidential: false`). The first-party
  app is seeded in the admin repo (`db/seeds/development/cli.rb`, client id
  `hardcoded_cli_client_id`), so browser login needs **no client secret** — and,
  because there's no `force_pkce`, **no PKCE migration is required** either. The
  CLI still sends PKCE on the wire (forward-compatible). For prod, register a
  real "ClickFunnels CLI" app and bake its uid into the release via
  `-ldflags "-X .../cmd.defaultClientID=<uid>"`.
...
To distribute: register a **production** "ClickFunnels CLI" OAuth app and bake
its client id in via `defaultClientID` ldflags; then GoReleaser → GitHub
Releases + Homebrew tap (+ a `curl|sh` installer; sign/notarize macOS for direct
downloads). The Doorkeeper PKCE migration and the spec naming/enum fixes are
**upstream in the admin repo** — nice-to-haves, not blockers; don't do them here.
